...
enum SyncRules {
    ...
    static func occurrenceComponents(sourceId: String, occurrenceDate: Date?, startDate: Date?, configId: UUID) -> (String, String, String) {
    ...
    static func needsUpdate(mode: SyncMode, template: String?, sourceTitle: String?, targetTitle: String?, sourceStart: Date?, targetStart: Date?, sourceEnd: Date?, targetEnd: Date?, sourceLocation: String?, targetLocation: String?) -> Bool {
        switch mode {
        case .full:
            return sourceTitle != targetTitle || sourceStart != targetStart || sourceEnd != targetEnd || (sourceLocation ?? "") != (targetLocation ?? "")
        case .blocker:
            let t = (template ?? "Busy").replacingOccurrences(of: "{sourceTitle}", with: sourceTitle ?? "")
    ...
    static func passesFilters(
        ...
        for rule in filters {
            ...
            case .includeAttendee:
                ...
                if !attendeeValues.contains(where: { contains($0, pattern: rule.pattern, cs: rule.caseSensitive) }) { return false }
            case .excludeAttendee:
                // Exclude if any attendee contains the pattern
                if attendeeValues.contains(where: { contains($0, pattern: rule.pattern, cs: rule.caseSensitive) }) { return false }

            case .durationLongerThan:
                if let mins = durationMinutes, let threshold = Int(rule.pattern.trimmingCharacters(in: .whitespaces)) {
    ...
    static func allowedByTimeWindows(start: Date?, isAllDay: Bool = false, windows: [TimeWindowUI]) -> Bool {
    ...
    static func safeToDeletePolicy(configId: UUID, targetCalendarId: String, eventCalendarId: String, marker: Marker?, hasMapping: Bool) -> Bool {
