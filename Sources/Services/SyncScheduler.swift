...
final class SyncScheduler: ObservableObject {
    ...
    private func runOnce() {
        coordinator.syncNow(configs: appState.syncs, defaultHorizonDays: appState.defaultHorizonDays, diagnosticsEnabled: appState.diagnosticsEnabled)
        appState.lastRunStatus = coordinator.lastStatus

        // Heuristic: if last run message indicates failure, increase backoff
        let didFail = coordinator.lastStatus.lastFailureAt != nil && (coordinator.lastStatus.lastSuccessAt == nil || (coordinator.lastStatus.lastFailureAt ?? Date()) > (coordinator.lastStatus.lastSuccessAt ?? .distantPast))
        if didFail {
