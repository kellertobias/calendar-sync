...
struct MenuContentView: View {
    ...
    private func lastSyncText() -> String {
        if let failure = appState.lastRunStatus.lastFailureAt,
           failure > (appState.lastRunStatus.lastSuccessAt ?? .distantPast) {
            let formatter = RelativeDateTimeFormatter()
            formatter.unitsStyle = .short
            let rel = formatter.localizedString(for: failure, relativeTo: Date())
            return "Last Error: \(rel) — click to Sync Now"
        } else if let last = appState.lastRunStatus.lastSuccessAt {
            let formatter = RelativeDateTimeFormatter()
            formatter.unitsStyle = .short
