...
struct SettingsView: View {
    ...
    var body: some View {
        NavigationSplitView {
            VStack(spacing: 0) {
                HStack(spacing: 12) {
                    Spacer()
                    Button { addSync() } label: { Label("New Sync", systemImage: "plus") }
                    Button(role: .destructive) { deleteSelectedSync() } label: { Label("Delete", systemImage: "trash") }
                    Spacer()
                }
                .padding(.horizontal)
                .padding(.vertical, 8)
            ...
            .safeAreaInset(edge: .bottom) {
                VStack {
                    Button { selection = .settings } label: { Label("Settings", systemImage: "gearshape") }
    ...
    private func deleteSelectedSync() {
        if case let .sync(id)? = selection { appState.deleteSync(id: id); selection = .settings }
    }
...
private struct SettingsDetail: View {
        ...
        ScrollView {
            ...
            VStack(alignment: .leading, spacing: 24) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    .padding(.vertical, 4)
                    Text("How far into the future to look for events when planning syncs if a sync doesn’t specify its own horizon.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                }
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Button(role: .destructive) { removeAllSyncedItems() } label: {
                        Label {
                    ...
                    .help("Removes all calendar items created by CalendarSync (identified via mapping + tag).")
                    .padding(.vertical, 4)

                    // Same styling for this destructive action: red icon, primary text.
                    Button(role: .destructive) { removeAllSyncs() } label: {
                        Label {
    ...
    private func removeAllSyncedItems() {
        coordinator.syncNow(configs: appState.syncs.map { cfg in
            var tmp = cfg
            tmp.enabled = true
            return tmp
        }, defaultHorizonDays: appState.defaultHorizonDays, diagnosticsEnabled: appState.diagnosticsEnabled)
    ...
    private func relaunchApp() {
        guard let bundlePath = Bundle.main.bundlePath.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed),
              let url = URL(string: "file://\(bundlePath)") else { return }
